...
// Branded Water Ripple Lake Effect Generator
...
// Periodically generate raindrops
let raindropInterval = setInterval(spawnRaindrop, 2200);

// Click interaction to generate ripples manually
document.addEventListener('DOMContentLoaded', () => {
    const landingView = document.getElementById('landing-view');
    if (landingView) {
        landingView.addEventListener('click', (e) => {
            // Prevent ripples when clicking buttons, tiles, navbar, etc.
            if (e.target.closest('.ecosystem-tile') || 
                e.target.closest('button') || 
                e.target.closest('.landing-navbar') || 
                e.target.closest('.landing-sidebar-drawer') || 
